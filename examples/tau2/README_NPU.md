...
This guide explains how to set up and run **tau2-airline** on an **Ascend NPU**
environment.
...
Clone the repository and install the benchmark:
...
Clone the **Areal** repository and check out the `ascend-dev` branch:

```bash
git clone <areal_repository>
cd AReal
git checkout ascend-dev
...
Make sure the file `qwen3_nonthinking.jinja` is available.
...
1. Update the YAML configuration file `config_30b_moe_airline_npu.yaml` with the correct
   paths.
1. Set `user_llm_base_url` to the **IP address of the node running the user simulator**.
1. Ensure the **vLLM server is started with `--enforce-eager`**.
1. In the YAML configuration, add:
