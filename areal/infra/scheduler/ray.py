...
class RayScheduler(Scheduler):
    ...
    def _create_forked_workers_internal(
        ...
        assert len(target_workers) % len(schedulings) == 0, f"check {len(target_workers)=} and {len(schedulings)=}"
        ...
        def get_device_envs(wgs: list[RayWorkerInfo]):
            ...
            resource_env[target_env_key] = ",".join([str(d) for d in sorted(visible_devices)])
            return resource_env
