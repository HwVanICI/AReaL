...
from mindspeed.ops.triton.utils import (
    ...
    for task_id in range(pid, total_tasks, num_programs):
        ...
        if HAS_WEIGHT:
            p_w = tl.make_block_ptr(
                weight, (W, D), (D, 1), (0, i_d * BD), (W, BD), (1, 0)
            )
        ...
        if not USE_INITIAL_STATE:
            for i_w in tl.static_range(-W + 1, 1):
                ...
                b_yi = tl.load(
                    x + bos * D + yi_offset_0 * D + yi_offset_1, mask=mask, other=0.0
                ).to(tl.float32)
                if HAS_WEIGHT:
            ...
            for i_w in tl.static_range(-W + 1, 1):
                ...
                b_yi = tl.load(
                    x + bos * D + o_x[:, None] * D + o_d, mask=m_x, other=0
                ).to(tl.float32)
        ...
        if is_tail_chunk:
            ...
            b_y_cast = tl.cast(
                b_y, dtype=y.dtype.element_ty, fp_downcast_rounding="rtne"
            )
            tl.store(
            ...
            p_y = tl.make_block_ptr(
                y + bos * D, (T_len, D), (D, 1), (i_t * BT, i_d * BD), (BT, BD), (1, 0)
            )
            tl.store(
                p_y,
    ...
    for task_id in range(pid, total_tasks, num_programs):  # pylint: disable=too-many-nested-blocks
        ...
        if not USE_FINAL_STATE and not USE_INITIAL_STATE:
            ...
            for i_w in tl.static_range(0, W):
                ...
                b_wdy = b_dy_sub
                if HAS_WEIGHT:
                    b_wdy = b_wdy * tl.extract_slice(
                        b_w, [W - i_w - 1, 0], [1, BD], [1, 1]
                    )

                    b_dw_sub = tl.sum(
                        b_dy_sub * b_x, 0
                    )  # [BT, BD] * [BT, BD] --> sum(0) = [BD]
                    b_dw = tl.insert_slice(
                        b_dw, b_dw_sub[None, :], [W - i_w - 1, 0], [1, BD], [1, 1]
                    )
            ...
            p_dw = tl.make_block_ptr(
                dw + i_tg * W * D, (W, D), (D, 1), (0, i_d * BD), (W, BD), (1, 0)
            )
            ...
            for i_w in tl.static_range(0, W):
                ...
                if HAS_WEIGHT:
                    ...
                    if USE_INITIAL_STATE:
                        ...
                        if ACTIVATION == "swish" or ACTIVATION == "silu":  # pylint: disable=consider-using-in
                            ...
                            b_dy_head = (
                                b_dy_head * b_ys_head * (1 + b_y_head * (1 - b_ys_head))
                            )
                        o_c = W - i_w + o_t
                ...
                b_wdy = (
                    ...
                    if not HAS_WEIGHT
                        ...
                        b_dy_shift
                        * tl.extract_slice(b_w, [W - i_w - 1, 0], [1, BD], [1, 1])
                    )
                )
                b_dx += b_wdy
                ...
                for i_w in tl.static_range(1, W):
                    ...
                    for i_t2 in tl.static_range(0, W - 1):
                        if i_t2 < i_w:
                            ...
                            if HAS_WEIGHT:
                                w_row = tl.extract_slice(
                                    b_w, [i_w - 1 - i_t2, 0], [1, BD], [1, 1]
                                )
                                b_dh0_s += tl.sum(dy0_row[None, :] * w_row, 0).to(
                                    tl.float32
                                )
                            else:
                                b_dh0_s += dy0_row
        ...
        if is_tail_chunk:
            ...
            b_dx_cast = tl.cast(
                b_dx, dtype=dx.dtype.element_ty, fp_downcast_rounding="rtne"
            )
            tl.store(
            ...
            p_dx = tl.make_block_ptr(
                dx + bos * D, (T_len, D), (D, 1), (i_t * BT, i_d * BD), (BT, BD), (1, 0)
            )
            tl.store(
                p_dx,
...
def causal_conv1d_fwd_impl(
    ...
    NUM_CORES = get_multiprocessor_count()
...
def causal_conv1d_bwd_impl(
    ...
    dw = (
        weight.new_empty(B * NT, W, D, dtype=torch.float)
    ...
    if initial_state is not None:
        ...
        dh0 = initial_state.new_zeros(
            min(eff_NT, triton.cdiv(W, BT)), *initial_state.shape
        )
    else:
        dh0 = None
...
def causal_conv1d_states_fwd_kernel(
    ...
    tl.store(
        final_state + i_n * D * W + o_d[:, None] * W + o_w, b_x, mask=m_d[:, None] & m_w
    )
...
def causal_conv1d_update_kernel_bdt_fwd(
    ...
    for task_id in tl.range(pid, total_task, pnum):
        ...
        if ti == 0:
            ...
            mask_x = (offset0_x < dim)[:, None] & (
                (offset1_x >= 0) & (offset1_x < seq_len)
            )[None, :]
            block_off_x = (
                bi * dim * seq_len + offset0_x[:, None] * seq_len + offset1_x[None, :]
            )
            x_b_tmp = tl.load(x_ptr + block_off_x, mask=mask_x, other=0)
            x_b = tl.insert_slice(
                st_b, x_b_tmp, (0, width - 1), (D_CHK_SIZE, T_CHK_SIZE), (1, 1)
            )
        else:
            offset0 = di * D_CHK_SIZE + tl.arange(0, D_CHK_SIZE)
            offset1 = (
                ti * T_CHK_SIZE - (width - 1) + tl.arange(0, T_CHK_SIZE + width - 1)
            )
            mask = (offset0 < dim)[:, None] & ((offset1 >= 0) & (offset1 < seq_len))[
                None, :
            ]
            block_off = (
                bi * dim * seq_len + offset0[:, None] * seq_len + offset1[None, :]
            )
        ...
        if t_end_off >= new_state_start_off:
            ...
            if t_off < -(width - 1):
                ...
                x_new_h = tl.extract_slice(
                    x_b, (-t_off, 0), (ST_STORE_HEAD_TILE_SIZE, D_CHK_SIZE), (1, 1)
                )
                ...
                nst_mask_h = (
                    (nst_off_y0 < dim)
                    & (nst_off_y1_h >= 0)
                    & (nst_off_y1_h < state_len)
                )
                block_ptr_h = (
                    bi * dim * state_len + nst_off_y0 * state_len + nst_off_y1_h
                )
                tl.store(conv_state_update_ptr + block_ptr_h, x_new_h, mask=nst_mask_h)
            else:
                x_new_s = tl.extract_slice(
                    x_b, (width - 1, 0), (T_CHK_SIZE, D_CHK_SIZE), (1, 1)
                )
                x_new_s = tl.trans(x_new_s, (1, 0))
                nst_off_y0 = di * D_CHK_SIZE + tl.arange(0, D_CHK_SIZE)[:, None]
                nst_off_y1 = width - 1 + t_off + tl.arange(0, T_CHK_SIZE)[None, :]
                nst_mask = (
                    (nst_off_y0 < dim) & (nst_off_y1 >= 0) & (nst_off_y1 < state_len)
                )
...
def causal_conv1d_update_bdt_impl(
    ...
    bias: torch.Tensor | None = None,
    activation: str | None = None,
    conv_state_indices: str | None = None,
):
    ...
    ST_STORE_HEAD_TILE_SIZE = (
        width
        if (seqlen % T_CHK_SIZE) > width
        else (width - seqlen % T_CHK_SIZE) % T_CHK_SIZE
    )
