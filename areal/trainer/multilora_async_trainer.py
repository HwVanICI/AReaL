...
class MultiLoRATrainerAsync:
    ...
    def train(
        ...
        # ============================================================
        # Initialize per-LoRA weight update metadata.
        # ============================================================
        self.weight_update_meta_mapping = {}
        ...
        for task_id in self.multi_task_manager.tasks:
            ...
            meta = deepcopy(self.weight_update_meta)
            meta.lora_name = task.lora_name

            self.weight_update_meta_mapping[task.lora_name] = meta

        # ============================================================
        # Helper: submit the next rollout for a task.
        # ============================================================
        def submit_next_rollout(task_id):
            task = self.multi_task_manager.get_task(task_id)
            data = task.next_train_batch()
            ...
            print(
                f"[ASYNC][SUBMIT] "
                f"lora={task.lora_name} "
                f"step={task.global_step}",
                flush=True,
            )

        # ============================================================
        # Initially submit one rollout for every LoRA.
        #
        # After this point there should always be one outstanding
        # rollout per active LoRA.
        # ============================================================
        rollout_inputs = []
        ...
        with ThreadPoolExecutor(
            ...
            futures = [
                ...
                for task_id, _, data in rollout_inputs
            ]

            for future in futures:
                future.result()

        for task_id, lora_name, _ in rollout_inputs:
            task = self.multi_task_manager.get_task(task_id)

            print(
                f"[ASYNC][INITIAL SUBMIT] "
                f"lora={lora_name} "
                f"step={task.global_step}",
                flush=True,
            )
        ...
        # Important:
        #
        #   We DO NOT wait for all LoRAs.
        #
        #   wait_for_rollouts() returns whichever rollout finishes
        #   first. We train that LoRA, update its weights, and then
        #   immediately submit its next rollout.
        #
        # Example:
        #
        #   A0 ────────────────┐
        #                       ↓
        #   B0 ────────→ train B0 → submit B1
        #
        #   A0 eventually → train A0 → submit A1
        #
        # ============================================================
        while True:
            if not self.multi_task_manager.has_running_tasks():
                print(
                    "No pending tasks.. waiting 5 seconds",
                    flush=True,
                )
                time.sleep(5)
                continue
            ...
            # --------------------------------------------------------
            # STEP 1:
            # Wait for ANY LoRA rollout to finish.
            # --------------------------------------------------------
            rollout_batch, lora_name, batch_submit_time = (
                self.wait_for_rollouts()
            )

            # Find the task associated with this LoRA.
            task_id = self.multi_task_manager.task_mapping[lora_name]
            task_state = self.multi_task_manager.get_task(task_id)

            assert task_state.lora_name == lora_name
            ...
            rollout_latency = time.time() - batch_submit_time

            print(
                f"[ASYNC][RECEIVE] "
                f"lora={lora_name} "
                f"step={global_step} "
                f"latency={rollout_latency:.2f}s",
                flush=True,
            )
            ...
            if rollout_batch:
                rewards = [
                    ...
                    if traj is not None
                    and traj.get("rewards") is not None
                ]
            ...
            # --------------------------------------------------------
            # STEP 4:
            # Construct LoRA-specific version metadata.
            #
            # IMPORTANT:
            # Never use self.weight_update_meta directly here.
            # Every version must be associated with the correct LoRA.
            # --------------------------------------------------------
            meta = self.weight_update_meta_mapping[lora_name]

            assert meta.lora_name == lora_name

            current_version = global_step
            new_version = global_step + 1

            versioned_meta_current = (
                meta.with_version_multilora(
                    current_version,
                    lora_name,
                )
            )
            ...
            assert (
                versioned_meta_current.lora_name
                == lora_name
            )

            assert (
                versioned_meta_new.lora_name
                == lora_name
            )
            ...
            # --------------------------------------------------------
            # STEP 5:
            # Load the correct LoRA version.
            # --------------------------------------------------------
            if global_step > 0:
                self.actor.update_lora(
                    versioned_meta_current,
                    global_step,
                    lora_name,
                )
            ...
            # --------------------------------------------------------
            # STEP 6:
            # Prepare rollout-dependent models.
            # --------------------------------------------------------
            ...
            if self.critic is not None:
                ...
                with (
                    ...
                    self.critic.get_device_stats().log(
                        "critic values"
                    )
            ...
            if self.ref is not None:
                ...
                with (
                    ...
                    for traj, logp in zip(
                        rollout_batch,
                        ref_logps,
                    ):
                        traj["ref_logp"] = logp

                    self.ref.get_device_stats().log(
                        "ref logp"
                    )
            ...
            # --------------------------------------------------------
            # Recompute proximal log probabilities.
            # --------------------------------------------------------
            if config.actor.should_compute_prox_logp():
                with (
                    ...
                    self.actor.get_device_stats().log(
                        "recompute logp"
                    )
            ...
            # --------------------------------------------------------
            # Wait for checkpoint staging before modifying params.
            # --------------------------------------------------------
            ...
            if (
                ...
                and global_step
                in config.memory_profiler.profile_steps
            ):
                self.actor.start_memory_profile(
                    config.memory_profiler.max_entries
                )
            ...
            if (
                ...
                log_dir = StatsLogger.get_log_path(
                    config.stats_logger
                )
                ...
                snapshot_dir = os.path.join(
                    log_dir,
                    "memory_snapshots",
                    f"{lora_name}_step_{global_step}",
                )
                ...
                self.actor.stop_memory_profile(
                    snapshot_dir
                )

                logger.info(
                    f"Memory snapshots saved to "
                    f"{snapshot_dir}"
                )
            ...
            # --------------------------------------------------------
            # Pause rollout while updating weights.
            # --------------------------------------------------------
            # self.rollout.pause()
            ...
            #
            # IMPORTANT:
            # Use versioned_meta_new from the LoRA-specific mapping.
            # --------------------------------------------------------
            with (
                stats_tracker.record_timing(
                    "update_weights"
                ...
                if versioned_meta_new.colocate_mode:
                    actor_stats = (
                        self.actor.export_stats()
                    )
                ...
                self.actor.set_version(
                    new_version
                )

                if self.critic is not None:
                    self.critic.set_version(
                        new_version
                    )

                self.rollout.set_version(
                    new_version
                )

                if self.eval_rollout is not None:
                    self.eval_rollout.set_version(
                        new_version
                    )
            ...
            with (
                ...
                if is_single_controller():
                    ...
                    if self.critic is not None:
                        self.critic.clear_batches(
                            rollout_batch,
                            adv_batch,
                        )

                    if self.ref is not None:
                        self.ref.clear_batches(
                            rollout_batch
                        )
            ...
            # --------------------------------------------------------
            # Increment THIS LoRA's step.
            #
            # Do this before submitting the next rollout so the next
            # rollout is associated with the new step.
            # --------------------------------------------------------
            task_state.global_step += 1

            # --------------------------------------------------------
            # Remove sufficiently old version.
            #
            # Keep two versions behind because another asynchronous
            # rollout may still be using an older version.
            # --------------------------------------------------------
            remove_version = current_version - 2
            ...
            if remove_version >= 1:
                ...
                print(
                    f"[ASYNC][CLEANUP] "
                    f"lora={lora_name} "
                    f"remove_version={remove_version} "
                    f"path={old_meta.path}",
                    flush=True,
                )

                self.remove_old_versioned_folder(
                    old_meta.path
                )
            ...
            # --------------------------------------------------------
            # Immediately submit the next rollout for THIS LoRA.
            #
            # Other LoRAs continue to have their own rollouts in flight.
            # --------------------------------------------------------
            submit_next_rollout(task_id)
